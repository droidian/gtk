...
This VS17 solution and the projects it includes are intented to be used
...
build/win32/vs17/README.txt file in glib for details where to unpack them.
...
It is recommended that one builds the dependencies with VS17 as far as
possible, especially those from and using the GTK+ stack (i.e. GLib,
Cairo, ATK, Pango, GDK-Pixbuf), so that crashes caused by mixing calls
to different CRTs can be kept at a minimum.

zlib, libpng, and Cairo do contain support for compiling under VS17
using VS project files and/or makefiles at this time of writing, For the
GTK+ stack, VS17 project files are either available under
$(srcroot)/build/vs17 in the case of GLib (stable/unstable), ATK
(stable/unstable) and GDK-Pixbuf (unstable), and should be in the next
unstable version of Pango.  There is no known official VS17 build
...
Unzip the binaries obtained from ftp.gnome.org in <root>\vs17\<PlatformName>,
and build the following, if not already done so: 

Note: put the resulting zlib, libpng, pcre and Cairo files as follows:
 .dll files: <root>\vs17\<PlatformName>\bin
 .lib files: <root>\vs17\<PlatformName>\lib
 .h files: <root>\vs17\<PlatformName>\include

The recommended build order for these dependencies:
(first unzip any dependent binaries downloaded from the ftp.gnome.org
 as described in the README.txt file in the build/win32/vs17 folder)
-zlib
-libpng
...
-(optional for GLib) PCRE (version 8.12 or later, use of CMake to
  build PCRE is recommended-see build/win32/vs17/README.txt of GLib)
-GLib **
...
appropriate location under <root>\vs17\<PlatformName>. For instance,
built DLLs go into <root>\vs17\<PlatformName>\bin, built LIBs into
<root>\vs17\<PlatformName>\lib and GTK+ headers into
<root>\vs17\<PlatformName>\include\gtk-3.0. This is then from where
...
needs to be found in <root>\vs17\<PlatformName>\, with the introspection files
...
<root>\vs17\<PlatformName>\share\gir-1.0 (the .gir file(s)) and
<root>\vs17\<PlatformName>\lib\girepository-1.0 (the .typelib files(s)) upon
...
**:Put the sources of the packages marked with ** in <root>\<package-
    source-tree>, and build with VS17 from there.
